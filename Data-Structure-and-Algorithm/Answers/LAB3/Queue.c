...
typedef struct _Node{
  ...
  int quantity; 
  int items; //items ID, this make the process of choosing items much easier, no need to mess around with strings as input choice
  struct _Node *next;
} node;
...
node* initItems(int val, char name[]){ //initialize all items with it corresponding name and capacity
...
void display(node*a,node*b,node*c){ //display the name and quantity of the 3 items
  printf("\nItem name: %s. Item ID: 1. Item quantity: %d",a->name_items,a->quantity);
  printf("\nItem name: %s. Item ID: 2. Item quantity: %d",b->name_items,b->quantity);
  printf("\nItem name: %s. Item ID: 3. Item quantity: %d\n",c->name_items,c->quantity);
}

void purchase(queue *I, node *a, node *b, node *c){ //checking for purchase requirements
  node *p = I->front;
  int count = 0;
  if(p == NULL){
    printf("\nThe Queue is empty");
  ...
  while(count<=(I->capacity)){
    ...
    if(p->items == 1 && a->quantity>0){
      if(p->quantity > a->quantity){ //if the person want more than the stock can offer
        ...
        printf("\n%s purchase success: %d %s",p->name_human,p->quantity,a->name_items);
    ...
    if(p->items == 2 && b->quantity>0){
      if(p->quantity > b->quantity){
        ...
        printf("\n%s purchase success: %d %s",p->name_human,p->quantity,b->name_items);
    ...
    if(p->items == 3 && c->quantity>0){
      if(p->quantity > c->quantity){
        ...
        printf("\n%s purchase success: %d %s",p->name_human,p->quantity,c->name_items);
